...
class TaskGroupExc(Exception):
    def __init__(self, msg: str, error: List[Exception]):
        self.msg = msg
        self.error = error
        super().__init__(msg)
...
class TaskGroup:
    """copy from https://github.com/python/cpython/blob/main/Lib/asyncio/taskgroups.py"""
    ...
    async def __aexit__(self, et, exc, tb):
        ...
        while self._unfinished_tasks:
            if self._on_completed_fut is None:
                self._on_completed_fut = self._loop.create_future()  # type: ignore
        ...
        if self._errors:
            ...
            me = TaskGroupExc("unhandled errors in a TaskGroup", errors)
            raise me from None
    ...
    def create_task(self, coro, *, name=None, context=None):
        ...
        if context is None:
            ...
            task = self._loop.create_task(coro, context=context)  # type: ignore
    ...
    def _on_task_done(self, task):
        ...
        if self._parent_task.done():  # type: ignore
            # Not sure if this case is possible, but we want to handle
            # it anyways.
            self._loop.call_exception_handler(  # type: ignore
                {  # type: ignore
        ...
        if not self._aborting and not self._parent_cancel_requested:
            ...
            self._parent_task.cancel()  # type: ignore
