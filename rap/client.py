...
class AsyncIteratorCall:
    ...
    async def __anext__(self):
        response: Response = await self._client._msg_request(self._method, *self._args, call_id=self._call_id)
...
class Client:
    ...
    async def _declare(self):
        ...
        if response.response_num != Constant.DECLARE_RESPONSE and response.body != body:
    ...
    async def _drop(self):
        ...
        if response.response_num != Constant.DROP_RESPONSE and response.body.get("call_id", "") != call_id:
    ...
    async def _base_request(self, request_num: int, header: dict, body: Any) -> Response:
        ...
        try:
            ...
            # TODO
            response: Response = await asyncio.wait_for(self._future_dict[msg_id], self._timeout)
            return response
        finally:
    ...
    async def raw_call(self, method: str, *args: Any) -> Any:
        ...
        response = await self._msg_request(method, *args)
        return response.body['result']
