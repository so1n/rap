    ...
    async def test_secret_middleware_method(self, rap_server: Server) -> None:
        ...
        async with process_client(CryptoProcessor("test", "keyskeyskeyskeys")) as rap_client:
            ...
            await rap_client.invoke_by_name(
                "modify_crypto_timeout", {"timeout": 10}, group=middleware.__class__.__name__
            )
            assert middleware._timeout == 10
            await rap_client.invoke_by_name(
                "modify_crypto_nonce_timeout", {"timeout": 11}, group=middleware.__class__.__name__
            )
            ...
            await rap_client.invoke_by_name(
                "load_aes_key_dict",
                {"aes_key_dict": {"test1": "1234567890123456"}},
                group=middleware.__class__.__name__,
            )
            await rap_client.invoke_by_name("remove_aes", {"key": "test1"}, group=middleware.__class__.__name__)
