...
## 客户端
客户端支持通过`invoke_by_name`和`invoke`方法来调用服务,但这样无法完整利用到TypeHint的功能,推荐使用`@client.register`注册函数再进行调用
...
async def main():
    ...
    # await async_sum(1,3) 实际上等于 await client.invoke_by_name('async_sum', 1, 2)
...
```
对于客户端, 建议使用`client.register`,不要使用`client.invoke`, `client.invoke_by_name`.
